...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 解压..."
		tar -xzf libsodium-${Libsodiumr_ver}-stable.tar.gz && cd libsodium-stable/
		echo -e "${Info} 编译安装..."
		./configure --disable-maintainer-mode && make && make check && make install
	...
	ldconfig
	cd .. && rm -rf libsodium-${Libsodiumr_ver}-stable.tar.gz && rm -rf libsodium-stable
...
————————————
  ...
  —— 说明：该功能适合于SSR服务端经常进程结束，启动该功能后会每分钟检测一次，当进程不存在则自动启动SSR服务端。" && echo
	...
	if [[ ${other_num} == "1" ]]; then
		wget -N --no-check-certificate "https://github.com/Mr-LongFly/gost/raw/master/tcp.sh"
		...
		if [[ ! -f "/usr/local/sbin/gost" ]]; then
			...
			echo "gost服务已经安装，请直接使用gost命令 !" && exit 1
		fi
